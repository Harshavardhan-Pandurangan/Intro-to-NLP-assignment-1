...
class NGram:
    ...
    def train(self):
        ...
        for seq in self.ngrams:
            ...
            self.probabilities[seq] = {k_: v/total for k_, v in self.ngrams[seq].items()}
    ...
    def good_turing(self):
        ...
        N_r = {k_: v for k_, v in sorted(N_r.items(), key=lambda item: item[0])}
...
if lm_type == 'n':
    ...
    generator_seed = input('Enter the sentence: ')
    tokenizer = Tokenizer(sentence=generator_seed)

    tokens = tokenizer.tokens[0]
    tokens.insert(0, '<s>')
    tokens.insert(0, '<s>')

    token = tuple(tokens[-2:])
    if token in ngram.probabilities:
        predicts = ngram.probabilities[token]

        probs = [[k_, v] for k_, v in sorted(predicts.items(), key=lambda item: item[1], reverse=True)]
        ...
        if token[1] in ngram.vocab:
            token_list = {}
            for key in ngram.probabilities.keys():
                if key[1] == token[1]:
                    token_list[key] = ngram.probabilities[key]

            prob_list = []
            for values in token_list.values():
                for k_, v in sorted(values.items(), key=lambda item: item[1], reverse=True):
                    prob_list.append([k_, v])

            probs = sorted(prob_list, key=lambda item: item[1], reverse=True)
            for i in range(int(k)):
                if i > len(probs):
                    print('No more predictions')
                    break
            ...
            prob_list = []
            for values in ngram.probabilities.values():
                for k_, v in sorted(values.items(), key=lambda item: item[1], reverse=True):
                    prob_list.append([k_, v])
    ...
    token = tuple(tokens[-2:])
    if token in ngram.probabilities_gt:
        predicts = ngram.probabilities_gt[token]
        ...
        if token[1] in ngram.vocab:
            token_list = {}
            for key in ngram.probabilities_gt.keys():
                if key[1] == token[1]:
                    token_list[key] = ngram.probabilities_gt[key]
            ...
            prob_list = []
            for values in ngram.probabilities_gt.values():
                for k_, v in sorted(values.items(), key=lambda item: item[1], reverse=True):
                    prob_list.append([k_, v])
    ...
    token = tuple(tokens[-2:])
    if token in ngram.probabilities_i:
        predicts = ngram.probabilities_i[token]
        ...
        if token[1] in ngram.vocab:
            token_list = {}
            for key in ngram.probabilities_i.keys():
                if key[1] == token[1]:
                    token_list[key] = ngram.probabilities_i[key]
            ...
            prob_list = []
            for values in ngram.probabilities_i.values():
                for k_, v in sorted(values.items(), key=lambda item: item[1], reverse=True):
                    prob_list.append([k_, v])
